...
class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private var mOfferCell:UserOffersListCell?
    private var mOfferDetails:OfferDetails?
    ...
    private func addSkeletonOfferNode() {
        let offerNode = ASDisplayNode {[weak self] () -> UIView in
            if let self = self, let cell = Bundle.main.loadNibNamed("OfferListCell", owner: self, options: nil)?.first as? UserOffersListCell {
                cell.backgroundColor = .clear
                cell.contentView.backgroundColor = .clear
                cell.hideRightView()
                cell.hideProgressBar(isHide:false)
                if let message = self.item {
                    let incoming = message.content.effectivelyIncoming(message.context.account.peerId)
//                    let messageTheme = incoming ? message.presentationData.theme.theme.chat.message.incoming : message.presentationData.theme.theme.chat.message.outgoing
                    print("incoming \(incoming)")
                    cell.setOfferForSentMessage(isSent:!incoming)
                }
                cell.btnBookNow.addTarget(self, action: #selector(self.onBookOffer), for: .touchUpInside)
                self.mOfferCell = cell
                return cell
            }
            return UIView()
        }
        mainContainerNode.removeFromSupernode()
        self.addSubnode(offerNode)
    }
    
    private func updateOfferNode(offerDetails:OfferDetails?) {
        if let cell = mOfferCell,let offer = offerDetails {
            mOfferDetails = offerDetails
            cell.hideProgressBar(isHide:true)
            cell.configureOffer(offer)
            cell.btnBookNow.addTarget(self, action: #selector(self.onBookOffer), for: .touchUpInside)
            
//            cell.onOfferTypeSelection = {(type,button) in
//
//            }
        }
    }
    ...
    @objc func onBookOffer() {
        ...
        if mOfferDetails == nil,let message = item?.message.text,let url = URL(string: message) {
            if let savedOffer = AppUtils.getOfferDetails(id: url.lastPathComponent) {
                showSchedules.mSchedules = savedOffer.schedules
            }
        } else {
            showSchedules.mSchedules = mOfferDetails?.schedules ?? []
        }
        showSchedules.currentSelectedScheduleId = ""
        showSchedules.onConfirmSchedule = {[weak self] schedule in
            print("show alert toast")
            if let millis = Double(schedule.form_time) {
                self?.showToastMessage(text: AppUtils.getScheduledDateDisplay(milliSeconds: millis),controller:self?.closestViewController)
            }
//            self?.mSelectedSchedule = schedule
        }
        self.closestViewController?.present(showSchedules, animated: true, completion: nil)
    }
    
    private func showToastMessage(text:String,controller:UIViewController?) {
    ...
    private func getOfferDetails(offerURL:URL) {
        
//        if let offerId = offerURL.lastPathComponent {
        print("getOfferDetails \(offerURL.lastPathComponent)")
        
        if let savedOffer = AppUtils.getOfferDetails(id: offerURL.lastPathComponent) {
            print("return saved offer")
            self.updateOfferNode(offerDetails: savedOffer)
//            self.addOfferNode(offerDetails: savedOffer)
        } else {
            PeyServiceController.shared.getOfferDetails(offerId: offerURL.lastPathComponent) {[weak self] (result, _) in
                switch result {
                    case .success(let response):
//                        self.addOfferNode(offerDetails: response.data)
                        self?.updateOfferNode(offerDetails: response.data)
                    case .failure(_):
                        break
                }
            }
    ...
    override func asyncLayout() -> (_ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation, ListViewItemApply, Bool) -> Void) {
        ...
        if item?.message.text.contains(Constants.BASE_URL) ?? false,let message = item?.message.text,let url = URL(string: message) {
//            print("add offer link \(url)")
            self.addSkeletonOfferNode()
            self.getOfferDetails(offerURL: url)
//            self.addOfferNode(offerDetails: nil)
        }
