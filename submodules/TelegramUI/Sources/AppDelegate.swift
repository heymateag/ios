...
@objc(AppDelegate) class AppDelegate: UIResponder, UIApplicationDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
        ...
        testIsLaunched = true
        print("login token \(AppUtils.getLoginToken())")
        ...
        |> deliverOnMainQueue).start(next: { context in
            ...
            if let authContextValue = self.authContextValue {
                ...
                if authContextValue.authorizationCompleted {
                    ...
                    print("authorizationCompleted in app delegate \(authContextValue)")
                    self.registerAsNewUser()
    ...
    private func registerAsNewUser() {
        print("registerAsNewUser")
        PeyServiceController.shared.registerNewUser(request: RegisterRequest()) { result, _ in
            switch result {
                case .success(let response):
                    print("new reg \(response)")
                case .failure(let error):
                    print("new reg error \(error) ")
            }
        }
    }
